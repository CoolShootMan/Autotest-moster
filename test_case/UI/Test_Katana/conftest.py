    ...
    if preserve_video:
        for page in pages:
            ...
            try:
                ...
                # 将视频文件放入allure报告
                allure.attach.file(_build_artifact_test_folder(pytestconfig, request, file_name),name=f"{request.node.name}-{human_readable_status}",attachment_type=allure.attachment_type.WEBM)
